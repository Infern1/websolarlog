...
class PDO extends Session
{
    ...
    public function get($type)
    {
    	...
    	if ($type != 'request_token' && $type != 'access_token') {
        	...
        	/**
			 * bean version to get tokens
        	 */
        	$bean =  \R::findOne($this->table,' userID = :userID ', array(':userID'=>$this->userID));
        	if($bean){
        		$token = $this->decrypt($bean['token']);
        		$_SESSION[$this->namespace][$type] = $bean['token'];
        		return $token;
        	}
