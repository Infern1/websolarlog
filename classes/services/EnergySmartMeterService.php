...
class EnergySmartMeterService {
    ...
    public function getInvoiceData(Device $device, $beginDate, $endDate) {
        ...
        $beans = R::find(self::$tbl, ' deviceId = :deviceId AND time > :beginDate AND time < :endDate ORDER BY time', array(':deviceId' => $device, ':beginDate' => $beginDate, ':endDate' => $endDate)
        );
        /* echo "<pre>";
          var_dump($beans);
          echo "</pre>"; */
    ...
    public function janitorDbCheck() {
        ...
        // set the device id
        R::exec("UPDATE EnergySmartMeter SET deviceId = invtnum WHERE invtnum is not NULL");
    }
