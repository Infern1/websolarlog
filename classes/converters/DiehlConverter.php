        ...
        foreach (json_decode($inputLine) as $key => $value){
        	if(is_array($value)){
        		foreach ($value as $keys => $values){
			        ...
			        if($values->path == 'eNEXUS_0066[s:17,t:1]' AND $values->value>0){ $live->I1A = ($values->value);}
			        if($values->path == 'eNEXUS_0055[s:17,t:1]' AND $values->value>0){ $live->I1V = ($values->value);}
			        if($values->path == 'eNEXUS_0065[s:17,t:1]' AND $values->value>0){ $live->I1P = ($values->value);}
			        if($values->path == 'eNEXUS_0066[s:17,t:1,p:1]' AND $values->value>0){ $live->GA = ($values->value);}
